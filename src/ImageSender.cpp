...
ImageSender::ImageSender(const char* hostname, int port) :
    hostname_ (hostname),
    port_(port),
    pic_num_(0),
    socket_fdesc_(0) {}

void ImageSender::ConnectToReceiver() {
...
void ImageSender::SendImageDims(int cols, int rows) {
...
void ImageSender::SendImage(cv::Mat& image) {
